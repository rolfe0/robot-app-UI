...
st.write("目前狀態：🟢 CSS 透明度切換引擎已就緒！徹底根治任何因材質綁定或引號截斷導致的 Bug。")
...
if os.path.exists(model_filename):
    ...
    # 4. 🚀 終極防錯：不使用三個引號的字串，改用單行陣列串接，徹底根除語法截斷錯誤！
    html_lines = [
        ...
        '    <button id="unlock-audio-btn" style="background-color: #00CC66; color: white; border: none; padding: 12px 20px; font-size: 16px; border-radius: 8px; cursor: pointer; margin-bottom: 10px; font-weight: bold; width: 100%;">🔊 系統啟動步驟：請先點擊此處解鎖喇叭 (啟用雙模型同步引擎)</button>',
        '    <div style="position: relative; width: 100%; height: 450px; background: #2a2a35; border-radius: 10px; overflow: hidden;">',
        '        <model-viewer id="model-idle" src="data:application/octet-stream;base64,__B64_MODEL__" interaction-prompt="none" camera-controls autoplay loop style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 1;"></model-viewer>',
        '        <model-viewer id="model-talk" src="data:application/octet-stream;base64,__B64_MODEL__" interaction-prompt="none" autoplay loop style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 2; opacity: 0; pointer-events: none; transition: opacity 0.05s ease;"></model-viewer>',
        '    </div>',
        '    <div style="background-color: #000000; width: 100%; padding: 10px; border-radius: 8px; margin-top: 10px; border: 1px solid #444;">',
        '        <p id="status-debug" style="color: #00FF00; font-size: 13px; font-family: monospace; margin: 0;">系統狀態
